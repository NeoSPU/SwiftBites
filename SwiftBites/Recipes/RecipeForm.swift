...
struct RecipeForm: View {
    ...
    init(mode: Mode) {
        ...
        case .add:
            ...
            _imageFileName = .init(initialValue: nil)
            _imagePreviewData = .init(initialValue: nil)
        case .edit(let recipe):
            ...
            _imageFileName = .init(initialValue: recipe.imageData)
            _imagePreviewData = .init(initialValue: nil)
        }
    }
    ...
    private func imagePicker(width: CGFloat) -> some View {
        PhotosPicker(selection: $imageItem, matching: .images) {
            if let imagePreviewData, let uiImage = UIImage(data: imagePreviewData) {
            ...
            } else if let imageFileName, let uiImage = ImageStorage.shared.loadImage(named: imageFileName) {
                Image(uiImage: uiImage)
                    .resizable()
                    .scaledToFill()
                    .frame(width: width)
    ...
    private var removeImage: some View {
        if imagePreviewData != nil || imageFileName != nil {
            Button(
                role: .destructive,
                action: {
                    if let file = imageFileName {
                        try? ImageStorage.shared.delete(named: file)
                    }
                    imageFileName = nil
